...
func count_recursive_children(allowed_children map[string][]BagAmount, bag_kind string) (count int) {

	children, _ := allowed_children[bag_kind]

	for _, child := range children {
		count += child.amount * (1 + count_recursive_children(allowed_children, child.kind))
	}
	return count
...
func main() {
	...
	fmt.Println(count_recursive_children(allowed_children, "shiny gold"))
